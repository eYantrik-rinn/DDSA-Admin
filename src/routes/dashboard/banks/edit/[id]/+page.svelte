<script lang="ts">
  ...
  // Initialize with a complete data structure to ensure all fields exist
  let defaultData = {
    bankName: '',
    classification: 'PVT',
    logoUrl: '',
  ...
  // Merge the existing data with default structure to ensure all fields are present
  let bankData = {
    ...defaultData,
    ...data.bank,
    eligibilityData: {
      ...defaultData.eligibilityData,
      ...(data.bank.eligibilityData || {})
    },
    processingFees: {
      ...defaultData.processingFees,
      ...(data.bank.processingFees || {})
    }
  };
  ...
  async function updateBank() {
    try {
      ...
      const response = await fetch(`/api/banks/${data.bank.id}`, {
        method: 'PUT',
        headers: {
      ...
      toasts.success('Bank updated successfully');
      goto('/dashboard/banks');
    } catch (error) {
      console.error('Error updating bank:', error);
      toasts.error('Failed to update bank');
      loading = false;
    }
  }
</script>
...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div class="flex items-center space-x-3">
      ...
      <div>
        <h1 class="text-3xl font-bold text-gray-900">Edit Bank: {bankData.bankName}</h1>
        <p class="mt-1 text-sm text-gray-500">Update bank information and eligibility criteria</p>
      </div>
    </div>
  </div>
  
  <!-- Form -->
  <form on:submit|preventDefault={updateBank} class="space-y-8 bg-white p-6 rounded-lg shadow-sm border border-gray-200">
